    ...
    () => {
      ...
      ee.initialize(
        ...
        () => {
          ...
          var prefire_end = "2023-01-30";
          var postfire_start = "2023-02-15";
          var postfire_end = "2023-02-23";
          // var platform = selectPlatform.getValue();
          // var forestBlock = selectBlock.getValue();

          var area = ee.Geometry(req.body);
          ...
          var mapId = dNBR.sldStyle(sld_intervals).getMap(visParams);

          // Get the map ID for the pre and post fire images
          var post_fire_mapId = post_cm_mos.getMap(sentinelVisParams);

          console.log(mapId.mapid, "mapid 1 ---->");
          console.log(post_fire_mapId.mapid, "mapid 2");

          try {
            // Seperate result into 8 burn severity classes
            var thresholds = ee.Image([
              -1000, -251, -101, 99, 269, 439, 659, 2000,
            ]);
            ...
            console.log("All pixel counts:", allpixels.getInfo());
...
const getGeeRaster = (req, res) => {
  ...
  try {
    ...
    var classified = dNBR.lt(thresholds).reduce(reducer).toInt();
    ...
    console.log("All pixel counts:", allpixels.getInfor());
    ...
    var areacount = function (cnr, name) {
      var singleMask = classified.updateMask(classified.eq(cnr)); // mask a single class
      //Map.addLayer(singleMask.eq(5), {}, "singel mask" + name);
      var stats = singleMask.reduceRegion({
        reducer: ee.Reducer.count(), // count pixels in a single class
        geometry: area,
        scale: 30,
  ...
  // // Seperate result into 8 burn severity classes
  // var thresholds = ee.Image([-1000, -251, -101, 99, 269, 439, 659, 2000]);
  // var classified = ee.Image(dNBR.lt(thresholds));
  ...
  //     return classified.select(name).rename("sum").toInt();
  //   })
  // );
  // classified = toCollection.sum().rename("sum");
